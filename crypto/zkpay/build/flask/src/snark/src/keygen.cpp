...
private:
  ...
  r1cs_constraint_system<FieldT> _constraint_system;
public:
  Keygen(string PK_PATH, string VK_PATH)
  {
...
void Keygen::generate_r1cs()
{
  ...
  _constraint_system = _pb->get_constraint_system();

}

void Keygen::output_keypair()
{
  mt19937_64 rand_src(time(0));
  FieldT tau;
  tau.set_ulong(rand_src());

  r1cs_ppzksnark_keypair<default_r1cs_ppzksnark_pp> keypair = r1cs_ppzksnark_generator<default_r1cs_ppzksnark_pp>(_constraint_system, tau);

  output_data_to_file(keypair.pk, _PK_PATH);
  output_data_to_file(keypair.vk, _VK_PATH);
}
...
int main(int argc, char *argv[])
{
  ...
  Keygen.generate_r1cs();
  Keygen.output_keypair();
